...
function App() {
  ...
  useEffect(()=>{
    if(!ready)return;
    try{localStorage.setItem('quiet-field-settings',JSON.stringify({version:2,master,mix,favorites,saved,reducedEffects,locale}));}catch{notify('偏好保存失败，请检查磁盘可用空间');}
  },[master,mix,favorites,saved,ready,reducedEffects,locale]);
  useEffect(()=>{
  ...
  return <div className={`app${playing?' is-playing':''}${reducedEffects?' reduced-effects':''}`}>
    ...
    <aside className="sidebar">
      <div className="brand"><img className="brand-icon" src="./icon.png" width="38" height="38" alt=""/><span>{t("静野")}</span></div><p className="tagline">{t("留一点安静给自己")}</p>
      ...
      <button className="about-button" onClick={()=>setModal({type:'about'})}><Info size={16}/>{t("关于声音与循环")}</button>
    </aside>
    <main>
      ...
      <section className="intro"><div><h1>{t("今天，听见宁静。")}</h1><p>{t("选几种声音，调成自己的节奏。")}</p></div><Visualizer engine={engine} playing={playing} disabled={reducedEffects}/></section>

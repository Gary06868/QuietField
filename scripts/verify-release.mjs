...
try{
 let page=await app.firstWindow();await page.getByRole('heading',{name:'今天，听见宁静。'}).waitFor();
 ...
 for(const value of ['null','[]','42','{','{"saved":[null],"mix":null,"master":"bad"}']){
  await page.evaluate(v=>localStorage.setItem('quiet-field-settings',v),value);await page.reload();await page.getByRole('heading',{name:'今天，听见宁静。'}).waitFor();assert.equal(await page.getByRole('slider',{name:'总音量',exact:true}).inputValue(),'50');
 ...
 await page.reload();await page.waitForSelector('.app.reduced-effects');assert.equal(await page.locator('canvas.ring').evaluate(c=>c.width),300);result.checks.push('lightweight mode persisted, spectrum drawing disabled');
 ...
 const saved=await app.evaluate(({BrowserWindow})=>BrowserWindow.getAllWindows()[0].getNormalBounds());await app.close();app=await launch();page=await app.firstWindow();await page.getByRole('heading',{name:'今天，听见宁静。'}).waitFor();
 ...
 for(let cycle=0;cycle<3;cycle++){await app.close();app=await launch();page=await app.firstWindow();await page.getByRole('heading',{name:'今天，听见宁静。'}).waitFor();const b=await app.evaluate(({BrowserWindow})=>BrowserWindow.getAllWindows()[0].getNormalBounds());assert.deepEqual(b,restored,'bounds must not grow on successive restarts');}result.checks.push('three additional restarts keep identical window bounds');

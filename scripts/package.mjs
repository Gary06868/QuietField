...
try{
 for(const name of ['dist','electron.cjs','preload.cjs','window-state.cjs','window-geometry.cjs','desktop-companion.cjs','desktop-state.cjs','desktop-updates.cjs','updates.cjs','update-worker.ps1','LICENSE','THIRD_PARTY_NOTICES.md'])await fs.cp(path.join(root,name),path.join(stage,name),{recursive:true});
